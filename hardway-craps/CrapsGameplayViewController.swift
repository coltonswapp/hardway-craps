...
class CrapsGameplayViewController: UIViewController {
    ...
    func setupDebugMenu() {
        // Create menu items for each total (2-12)
        var rollMenuItems: [UIMenuElement] = []
        for total in 2...12 {
            let action = UIAction(title: "\(total)") { [weak self] _ in
                self?.flipDiceContainer.rollFixedTotal(total)
            }
            rollMenuItems.append(action)
        }
        
        let fixedRollMenu = UIMenu(title: "Fixed Roll", children: rollMenuItems)
        ...
        // Create Show Playstyle toggle action
        let showPlaystyleAction = UIAction(
            title: showPlaystyle ? "Hide Playstyle" : "Show Playstyle",
            image: UIImage(systemName: showPlaystyle ? "eye.slash" : "eye")
        ) { [weak self] _ in
            self?.togglePlaystyle()
        }
        
        // Create End Session action
        let endSessionAction = UIAction(title: "End Session", image: UIImage(systemName: "stop.circle"), attributes: .destructive) { [weak self] _ in
            self?.endSessionTapped()
        }
        
        let menu = UIMenu(children: [fixedRollMenu, showDetailsAction, showPlaystyleAction, endSessionAction])
        
        let menuButton = UIBarButtonItem(image: UIImage(systemName: "line.3.horizontal"), menu: menu)
    ...
    private func handleRollResult(die1: Int, die2: Int, total: Int) {
        ...
        if !winningBets.isEmpty {
            ...
            showBetResultContainer(amount: totalWinnings, isWin: true)
        ...
        if case .sevenOut = event {
            let totalBet = getAllBettingControls().reduce(0) { $0 + $1.betAmount }
            if totalBet > 0 {
                showBetResultContainer(amount: totalBet, isWin: false)
            }
        }
    ...
    private func showBetResultContainer(amount: Int, isWin: Bool) {
        let betResultContainer = BetResultContainer()
        view.addSubview(betResultContainer)
        
        NSLayoutConstraint.activate([
            betResultContainer.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            betResultContainer.centerYAnchor.constraint(equalTo: view.centerYAnchor)
        ])
        
        // Bring to front so it's visible
        view.bringSubviewToFront(betResultContainer)
        
        // Animate the bet result container independently
        betResultContainer.animateToAmount(amount, isWin: isWin)
