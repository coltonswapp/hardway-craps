...
final class BlackjackHandPlayground: UIViewController {
    ...
    private func setupHandsArea() {
        ...
        handsStackViewWidthConstraint = handsStackView.widthAnchor.constraint(equalToConstant: singleHandWidth)

        view.addSubview(activeHandDot)
        activeHandDotCenterXConstraint = activeHandDot.centerXAnchor.constraint(equalTo: compactHandView.centerXAnchor)
        activeHandDotBottomConstraint = activeHandDot.bottomAnchor.constraint(equalTo: compactHandView.handView.cardContainer.topAnchor, constant: -6)

        playerBalanceViewCenterXConstraint = playerBalanceView.centerXAnchor.constraint(equalTo: compactHandView.centerXAnchor)

        NSLayoutConstraint.activate([
            playerBalanceView.bottomAnchor.constraint(equalTo: handsContainerView.topAnchor, constant: -8),
            playerBalanceViewCenterXConstraint!,

            handsContainerView.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor, constant: handAreaMargin),
            handsContainerView.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -handAreaMargin),
    ...
    private func updatePlayerBalanceViewPosition() {
        ...
        if count >= 2 {
            playerBalanceViewCenterXConstraint = playerBalanceView.centerXAnchor.constraint(equalTo: handsStackView.centerXAnchor)
        } else {
            playerBalanceViewCenterXConstraint = playerBalanceView.centerXAnchor.constraint(equalTo: compactHandView.centerXAnchor)
        }
    ...
    @objc private func handTapped(_ sender: UITapGestureRecognizer) {
        guard let hand = sender.view as? CompactPlayerHandView,
              playerHands.count >= 2 else { return }
        let index = hand.tag
    ...
    private func setupButtons() {
        ...
        let firstRow = UIStackView(arrangedSubviews: [dealButton, hitButton, splitButton])
        ...
        buttonsStackView = UIStackView(arrangedSubviews: [firstRow, clearButtonStackView])
        ...
        NSLayoutConstraint.activate([
            ...
            buttonsStackView.heightAnchor.constraint(equalToConstant: 96)
        ])
    }
    ...
    @objc private func splitTapped() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
            self?.dealOneToSplitPair(left: hand, right: newHand)
        }
    }

    private func dealOneToSplitPair(left: CompactPlayerHandView, right: CompactPlayerHandView) {
        let deckCenter = deckCenterInView
        left.dealCard(randomCard(), from: deckCenter, in: view)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
            guard let self = self else { return }
            right.dealCard(self.randomCard(), from: deckCenter, in: self.view)
        }
    }
    ...
    private func updateActiveHandDot() {
        ...
        activeHandDotCenterXConstraint = activeHandDot.centerXAnchor.constraint(equalTo: activeHand.centerXAnchor)
        activeHandDotCenterXConstraint?.isActive = true
        activeHandDotBottomConstraint?.isActive = false
        activeHandDotBottomConstraint = activeHandDot.bottomAnchor.constraint(equalTo: activeHand.handView.cardContainer.topAnchor, constant: -6)
