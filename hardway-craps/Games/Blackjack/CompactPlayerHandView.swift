...
final class CompactPlayerHandView: UIView {
  ...
  private func setupView() {
    ...
    NSLayoutConstraint.activate([
      ...
      totalLabel.trailingAnchor.constraint(
        equalTo: handView.cardContainer.leadingAnchor, constant: -16),
      ...
      totalLabel.widthAnchor.constraint(greaterThanOrEqualToConstant: 24),
      ...
      betControl.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -4),
      ...
      contentStackView.leadingAnchor.constraint(
        greaterThanOrEqualTo: handView.leadingAnchor, constant: 8),
      contentStackView.trailingAnchor.constraint(
        lessThanOrEqualTo: handView.trailingAnchor, constant: -8),
  ...
  private func updateTotalLabel() {
    ...
    // Show label only when we have 2+ cards; hide for 1 card or less
    let shouldShow = cards.count >= 2
  ...
  private static func calculateBlackjackTotals(for cards: [BlackjackHandView.Card]) -> (
    primary: Int, alternative: Int?
  ) {
    var aceCount = 0
    var nonAceTotal = 0
    for card in cards {
  ...
  func discardCards(
    ...
    // Fade out totalLabel when discarding cards
    UIView.animate(
      withDuration: 0.3,
      animations: {
    ...
    handView.discardCards(to: endPoint, in: containerView) { [weak self] in
      ...
      // Restore alpha after updating label (will be hidden if no cards)
  ...
  func dealCardFaceDown(
    _ card: BlackjackHandView.Card, from startPoint: CGPoint, in containerView: UIView
  ) {
  ...
  // MARK: - Action Broadcast Animation

  /// Spawns a label that floats upward while fading out, staying attached to this hand view.
  func broadcastAction(_ action: String) {
    let cardFrame = handView.cardContainer.frame
    let startCenter = CGPoint(x: cardFrame.midX, y: cardFrame.minY - 10)

    let actionLabel = UILabel()
    actionLabel.text = action
    actionLabel.font = .systemFont(ofSize: 16, weight: .semibold)
    actionLabel.textColor = .white
    actionLabel.textAlignment = .center
    actionLabel.alpha = 1.0
    actionLabel.sizeToFit()

    let randomRotation = Double.random(in: -10...10) * .pi / 180.0

    actionLabel.center = startCenter
    // Start slightly smaller, then scale up as it moves
    actionLabel.transform = CGAffineTransform(rotationAngle: randomRotation)
      .concatenating(CGAffineTransform(scaleX: 0.8, y: 0.8))
    addSubview(actionLabel)

    UIView.animate(
        withDuration: 1.2, delay: 0, options: [.curveEaseOut],
      animations: {
        // Scale up to slightly larger than normal, then fade out
        actionLabel.transform = CGAffineTransform(rotationAngle: randomRotation)
  ...
  // MARK: - Touch handling (tap-to-hit, same as PlayerHandView)

  /// Deliver taps in the card area to the hand view so tap-to-hit works when inside a scroll view or with overlapping layout.
  override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
    let pointInHandView = convert(point, to: handView)
    let pointInCardContainer = handView.cardContainer.convert(pointInHandView, from: handView)
    let inCardArea = handView.cardContainer.bounds.contains(pointInCardContainer)

    print(
      "🎯 [CompactPlayerHandView] hitTest point=\(point) inCardArea=\(inCardArea) containerBounds=\(handView.cardContainer.bounds) handView.frame=\(handView.frame) handView.isUserInteractionEnabled=\(handView.isUserInteractionEnabled) self.bounds=\(bounds)"
    )

    if inCardArea {
      let result = handView.hitTest(pointInHandView, with: event) ?? handView
      print(
        "🎯 [CompactPlayerHandView] → forwarding to handView, got: \(type(of: result)) (\(result))")
      return result
    }
    let result = super.hitTest(point, with: event)
    print(
      "🎯 [CompactPlayerHandView] → super.hitTest got: \(String(describing: result.map { type(of: $0) }))"
    )
    return result
  }
}
