...
final class BaccaratCardView: UIView {
    ...
    private static let baseSuitSize: CGFloat = 36
    ...
    private static let baseValueFontSize: CGFloat = 48
    ...
    private func setupView() {
        ...
        layer.masksToBounds = false // Don't clip for 3D transforms
        layer.borderWidth = 1
        layer.borderColor = UIColor.black.withAlphaComponent(0.1).cgColor
        clipsToBounds = true // Still clip subviews to card bounds
        ...
        backView.alpha = 1 // Start face down

        valueLabel.translatesAutoresizingMaskIntoConstraints = false
        valueLabel.font = .systemFont(ofSize: Self.baseValueFontSize, weight: .semibold)
        valueLabel.textAlignment = .right
        valueLabel.clipsToBounds = false
        valueLabel.alpha = 0 // Start hidden

        suitImageView.translatesAutoresizingMaskIntoConstraints = false
        suitImageView.contentMode = .scaleAspectFit
        suitImageView.alpha = 0 // Start hidden
