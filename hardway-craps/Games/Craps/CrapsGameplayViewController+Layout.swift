...
extension CrapsGameplayViewController {
    ...
    private var landscapeLeftContainer: UIView {
        get {
            if let existing = objc_getAssociatedObject(self, &AssociatedKeys.landscapeLeftContainer) as? UIView {
                return existing
            }
            let container = UIView()
            container.translatesAutoresizingMaskIntoConstraints = false
            objc_setAssociatedObject(self, &AssociatedKeys.landscapeLeftContainer, container, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            return container
        }
    }
    ...
    private var landscapeRightContainer: UIView {
        get {
            if let existing = objc_getAssociatedObject(self, &AssociatedKeys.landscapeRightContainer) as? UIView {
                return existing
            }
            let container = UIView()
            container.translatesAutoresizingMaskIntoConstraints = false
            objc_setAssociatedObject(self, &AssociatedKeys.landscapeRightContainer, container, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            return container
        }
    }
    ...
    private var iPhoneConstraints: [NSLayoutConstraint] {
        get { objc_getAssociatedObject(self, &AssociatedKeys.iPhoneConstraints) as? [NSLayoutConstraint] ?? [] }
        set { objc_setAssociatedObject(self, &AssociatedKeys.iPhoneConstraints, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
    
    private var iPadPortraitConstraints: [NSLayoutConstraint] {
        get { objc_getAssociatedObject(self, &AssociatedKeys.iPadPortraitConstraints) as? [NSLayoutConstraint] ?? [] }
        set { objc_setAssociatedObject(self, &AssociatedKeys.iPadPortraitConstraints, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
    
    private var iPadLandscapeConstraints: [NSLayoutConstraint] {
        get { objc_getAssociatedObject(self, &AssociatedKeys.iPadLandscapeConstraints) as? [NSLayoutConstraint] ?? [] }
        set { objc_setAssociatedObject(self, &AssociatedKeys.iPadLandscapeConstraints, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
    
    private var currentActiveConstraints: [NSLayoutConstraint] {
        get { objc_getAssociatedObject(self, &AssociatedKeys.currentActiveConstraints) as? [NSLayoutConstraint] ?? [] }
        set { objc_setAssociatedObject(self, &AssociatedKeys.currentActiveConstraints, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
    ...
    private var previousLayoutMode: LayoutMode? {
        ...
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.previousLayoutMode, newValue?.rawValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    ...
    private var iPadContainersCreated: Bool {
        ...
        set { objc_setAssociatedObject(self, &AssociatedKeys.iPadContainersCreated, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
    ...
    func applyLayout(for mode: LayoutMode) {
        ...
        if mode == .iPadLandscape {
            ...
            print("  topStackView frame: \(topStackView.frame)")
            print("  topStackView bounds: \(topStackView.bounds)")
            ...
            print("  bottomStackView frame: \(bottomStackView.frame)")
    ...
    private func buildIPhoneConstraints() -> [NSLayoutConstraint] {
        ...
        constraints.append(contentsOf: [
            ...
            betsContainerView.heightAnchor.constraint(equalToConstant: UIDevice.current.userInterfaceIdiom == .pad ? 188 : 156),
            ...
            betsScrollView.heightAnchor.constraint(equalToConstant: UIDevice.current.userInterfaceIdiom == .pad ? 168 : 136),
        ...
        let gameContainerConstraints = [
            ...
            gameContainerView.bottomAnchor.constraint(equalTo: bottomStackView.topAnchor, constant: -24)
        ]
        constraints.append(contentsOf: gameContainerConstraints)
        print("📐 iPhone: gameContainerView constraints - top: betsContainer.bottom+12, bottom: bottomStack.top-24")
        ...
        constraints.append(contentsOf: [
            ...
            chipSelector.heightAnchor.constraint(equalToConstant: 60)
    ...
    private func buildIPadPortraitConstraints() -> [NSLayoutConstraint] {
        ...
        constraints.append(contentsOf: [
            ...
            betsContainerView.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            betsContainerView.topAnchor.constraint(equalTo: topStackView.bottomAnchor, constant: 12),
            betsContainerView.bottomAnchor.constraint(lessThanOrEqualTo: gameContainerView.topAnchor, constant: -50)
        ])
        ...
        let gameContainerConstraints = [
            gameContainerView.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            gameContainerView.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            gameContainerView.topAnchor.constraint(equalTo: betsContainerView.bottomAnchor, constant: 50),
            gameContainerView.bottomAnchor.constraint(equalTo: bottomStackView.topAnchor, constant: -24)
        ]
        constraints.append(contentsOf: gameContainerConstraints)
        print("📐 iPad Portrait: gameContainerView constraints - top: betsContainer.bottom+50, bottom: bottomStack.top-24")
    ...
    private func buildIPadLandscapeConstraints() -> [NSLayoutConstraint] {
        ...
        constraints.append(contentsOf: [
            ...
            topStackView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -16),
            topStackTopConstraint,
            topStackView.heightAnchor.constraint(lessThanOrEqualToConstant: 100), // Max height in landscape
            instructionLabel.heightAnchor.constraint(greaterThanOrEqualToConstant: 60),
            instructionLabel.heightAnchor.constraint(lessThanOrEqualToConstant: 80), // Max height for instruction label
            currentBetView.widthAnchor.constraint(equalToConstant: 100)
        ])
        ...
        constraints.append(contentsOf: [
            ...
            leftC.bottomAnchor.constraint(equalTo: bottomStackView.topAnchor, constant: -24),
            ...
            rightC.bottomAnchor.constraint(equalTo: bottomStackView.topAnchor, constant: -24), // Independent!
        ...
        constraints.append(contentsOf: gameContainerConstraints)
        print("📐 iPad Landscape: gameContainerView constraints - fills landscapeLeftContainer completely")
        print("📐 landscapeLeftContainer: top=topStack.bottom+12, bottom=bottomStack.top-8, width=50%")
        print("📐 landscapeRightContainer: top=topStack.bottom+12, bottom=bottomStack.top-8, width=50% (INDEPENDENT)")
        ...
        // Bottom stack view: bottom-left, sized to content
        let chipSelectorHeight: CGFloat = 60
        constraints.append(contentsOf: [
            bottomStackView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 16),
            bottomStackView.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -8),
